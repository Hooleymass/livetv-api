...
// Define your GraphQL schema and resolver functions
const schema = buildSchema(`
  ...
  type Query {
    channels(cursor: Int, limit: Int): ChannelConnection!
...
const root = {
  channels: ({ cursor = 0, limit = 10 }) => {
    ...
    const hasNextPage = endIndex < mergedData.length;
    const endCursor = hasNextPage ? endIndex : null;
    return {
      edges: paginatedData.map(node => ({ cursor: node.id, node })),
